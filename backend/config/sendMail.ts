...
const sendEmail = async (to: string, url: string, txt: string) => {
  ...
  try {
    ...
    const mailOptions = {
      from: SENDER_MAIL,
      to: to,
      subject: "SneakersLoop",
      html: `
              <div style="max-width: 700px; margin:auto; border: 10px solid #ddd; padding: 50px 20px; font-size: 110%;">
              <h2 style="text-align: center; text-transform: uppercase;color: teal;">Welcome to SneakersLoop!</h2>
              <p>Congratulations! You're almost set to start using SpeakUP.
